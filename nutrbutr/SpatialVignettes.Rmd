...
for (f in filenames){
  ...
  low_res <-aggregate(raster_object,10,modal)
  saveRDS(low_res, paste(f,".low_res.rds", sep = ""))
  # Then in chunks of 100, Run Opperation to Discover What is in each 2014 boundary
  nicole  <- break_operation(low_res,kern.boundaries.t.major, 100)
  saveRDS(nicole, paste(f,".summary.rds", sep = ""))
  final_summaries[[f]] <- nicole
}
